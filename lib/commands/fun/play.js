...
var playCommand = {
	...
	process: function(bot, msg, args){
		...
		YoutubeTrack.getInfoFromVid(args[0], msg, function(err, video){
			...
			Queue.push(video);
			bot.sendMessage({to: msg.channel.id, message: "I have queued **" + video.title + "** it's in position " + Queue.length });
			if (Queue.length == 1){
				playNext(bot, msg);
...
function playNext(bot, msg){
	...
	bot.getAudioContext({channel: member.voice_channel_id, stereo: true}, function(stream){
		...
		ffmpeg.stdout.once("readable", function(){
			...
			bot.sendMessage({to: msg.channel.id, message: video.prettyPrint()});
